...
class ReceivedProduct(models.Model):
    actor = models.ForeignKey(User, related_name='receive_by')
    ...
    def generate_invoice_number(self):
        ...
        total = ReceivedProduct.objects.filter(created_at__year=year, created_at__month=month).count()
        return 'REC-%s%s%0.6d' % (year, month, total + 1)

    def save(self):
        if not self.pk :
           ### we have a newly created object, as the db id is not set
           self.invoice_number = self.generate_invoice_number()
        super(ReceivedProduct , self).save()

class ReceivedProductDetail(models.Model):
    received_product = models.ForeignKey(ReceivedProduct, related_name='details')
